...
def get_afternoon_candles(df: pd.DataFrame, trade_date: dt.date) -> pd.DataFrame:
    """Extract 12 PM – 4 PM CT candles for a given date.
    The 3 PM candle starts at 3 PM and closes at 4 PM, so the
    window must extend to 4 PM to include it."""
    prior = get_prior_trading_day(trade_date)
    start = CT.localize(dt.datetime.combine(prior, dt.time(12, 0)))
...
def get_rth_candles(df: pd.DataFrame, trade_date: dt.date) -> pd.DataFrame:
    """Extract full RTH session: 8:30 AM – 3:00 PM CT for a given date."""
    prior = get_prior_trading_day(trade_date)
    start = CT.localize(dt.datetime.combine(prior, dt.time(8, 30)))
    end = CT.localize(dt.datetime.combine(prior, dt.time(15, 0)))
    mask = (df.index >= start) & (df.index < end)
    return df.loc[mask].copy()

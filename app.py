...
with tab_dashboard:
    ...
    if today_events:
        ...
        st.markdown(f'<div class="macro-warn" style="border-color:{worst_color};">'
            f'<span class="prophet-label" style="color:{worst_color}"><span class="icon">&#9888;</span>MACRO EVENT TODAY</span><br>'
            f'<span class="prophet-data" style="color:{worst_color}">{event_names}</span><br>'
            f'<span class="prophet-data-muted">Recommendation: {today_macro_rec}{blackout_msg}</span>'
    ...
    pnl_col, countdown_col, readiness_col = st.columns([2, 1, 1])
    ...
    with sig_col:
        ...
        <div class="prophet-card">
            <span class="prophet-label"><span class="icon">⚡</span>ACTIVE SIGNAL</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    if signals:
        ...
        st.markdown("""
        <div class="prophet-card">
            <span class="prophet-label"><span class="icon">📈</span>TODAY'S SIGNALS</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        for sig_time, sig in signals:
            ...
            st.markdown(
                ...
                f'<span class="prophet-data" style="color:{COLORS["text_muted"]}; min-width:80px;">'
                f'{sig_time.strftime("%I:%M %p")}'
                f'</span>'
                f'<span class="prophet-data" style="color:{color}; font-weight:600; min-width:60px;">'
                f'{sig.direction}'
                f'</span>'
                f'<span class="prophet-data-muted">'
                f'at {sig.entry_line} · {sig.entry_price:,.2f}'
                f'</span>'
                f'<span class="prophet-data" style="color:{COLORS["accent_gold"]};">'
...
with tab_options:
    st.markdown(
        ...
        '<span class="prophet-label">'
        '<span class="icon" style="font-size:2rem;">&#129518;</span>'
        '0DTE SPX OPTIONS CALCULATOR</span>'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    if "opt_result" in st.session_state:
        ...
        with r1:
            st.markdown(
                ...
                '<span class="prophet-price-sm">' + f"{opt.strike:.0f}" + '</span><br>'
        ...
        with r2:
            st.markdown(
                ...
                '<span class="prophet-price-sm">$' + f"{opt.premium_entry:.2f}" + '</span><br>'
                '<span class="prophet-data-muted">per contract</span>'
                '</div>',
                unsafe_allow_html=True,
            )
        with r3:
            st.markdown(
                ...
                '<span class="prophet-price-sm" style="color:'
                + COLORS["bullish"] + ';">$' + f"{opt.net_profit:+,.0f}" + '</span><br>'
                '<span class="prophet-data-muted">Premium &rarr; $'
        ...
        with g1:
            st.markdown(
                '<div class="prophet-card" style="text-align:center;">'
                '<span class="prophet-label">DELTA</span><br>'
                '<span class="prophet-price-sm" style="color:' + _delta_color + ';">'
                + f"{opt.delta_approx:.4f}" + '</span><br>'
                '<span class="prophet-data-muted">$ per 1pt SPX move</span>'
                '</div>',
                unsafe_allow_html=True,
            )
        with g2:
            st.markdown(
                '<div class="prophet-card" style="text-align:center;">'
                '<span class="prophet-label">GAMMA</span><br>'
                '<span class="prophet-price-sm" style="color:' + _gamma_color + ';">'
                + f"{opt.gamma_approx:.6f}" + '</span><br>'
                '<span class="prophet-data-muted">Delta change per 1pt</span>'
                '</div>',
                unsafe_allow_html=True,
            )
        with g3:
            st.markdown(
                '<div class="prophet-card" style="text-align:center;">'
                '<span class="prophet-label">THETA</span><br>'
                '<span class="prophet-price-sm" style="color:' + _theta_color + ';">'
                + f"{opt.theta_approx:.2f}" + '</span><br>'
                '<span class="prophet-data-muted">$/day time decay</span>'
                '</div>',
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            '<div class="prophet-card" style="text-align:center;padding:1.2rem;">'
            '<span class="prophet-label">'
            '<span class="icon" style="font-size:2rem;">&#127919;</span>'
            ...
            + '</span><br>'
            '<span class="prophet-data-muted" style="font-size:1rem;">'
        ...
        if _is_deep_otm and _is_low_tte:
            st.markdown(
                '<div class="prophet-card" style="text-align:center;padding:1rem;'
                'border:2px solid ' + COLORS["bearish"] + ';'
                'background:rgba(255,0,85,0.12);">'
                '<span style="font-size:2rem;">&#9888;&#65039;</span><br>'
                '<span class="prophet-label" style="color:'
                + COLORS["bearish"] + ';">'
                'EXTREME THETA DECAY WARNING</span><br><br>'
                '<span class="prophet-data" style="color:'
                + COLORS["bearish"] + ';">'
...
with tab_macro:
    ...
    with wc2:
        ...
        st.markdown(
            ...
            '<span class="prophet-price-sm" style="color:' + e_color + ';">'
    ...
    with wc3:
        ...
        st.markdown(
            ...
            '<span class="prophet-price-sm" style="color:' + COLORS["bullish"] + ';">'
    ...
    if upcoming:
        for event in upcoming:
            ...
            st.markdown(
                ...
                '<span class="prophet-data" style="font-size:0.9rem;">'
        ...
        st.markdown(
            ...
            '<span class="prophet-data-muted">No macro events in the next '
    ...
    if news_items:
        ...
        st.markdown(
            '<div class="prophet-card">' + news_rows_html + '</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<div class="prophet-card" style="text-align:center;padding:1.5rem;">'
            '<span class="prophet-data-muted">'
...
with tab_backtest:
    ...
    st.markdown("""
    <div class="prophet-card prophet-card-gold">
        <span class="prophet-label"><span class="icon">🔬</span>STRATEGY BACKTESTER</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    if "bt_results" in st.session_state:
        ...
        if r.total_trades > 0:
            ...
            exp_html = '<div class="prophet-card" style="text-align:center;border:1px solid ' + exp_color + ';">'
            exp_html += '<span class="prophet-label">EXPECTANCY PER TRADE</span><br>'
            exp_html += '<span class="prophet-price-sm" style="color:' + exp_color + ';">$' + f"{expectancy:+,.2f}" + '</span><br>'
            exp_html += '<span class="prophet-data-muted" style="font-size:0.75rem;">'
        ...
        if r.trades:
            ...
            with bw_col1:
                bt_html = '<div class="prophet-card" style="border-left:3px solid ' + COLORS["bullish"] + ';">'
                bt_html += '<span class="prophet-label">BEST TRADE</span><br>'
                bt_html += '<span class="prophet-data" style="color:' + COLORS["bullish"] + ';font-size:1.1rem;">$' + f"{best_trade.result_dollars:+,.0f}" + '</span><br>'
                bt_html += '<span class="prophet-data-muted" style="font-size:0.75rem;">'
            ...
            with bw_col2:
                wt_html = '<div class="prophet-card" style="border-left:3px solid ' + COLORS["bearish"] + ';">'
                wt_html += '<span class="prophet-label">WORST TRADE</span><br>'
                wt_html += '<span class="prophet-data" style="color:' + COLORS["bearish"] + ';font-size:1.1rem;">$' + f"{worst_trade.result_dollars:+,.0f}" + '</span><br>'
                wt_html += '<span class="prophet-data-muted" style="font-size:0.75rem;">'
        ...
        if r.total_trades > 1:
            ...
            with sh_col1:
                sh_html = '<div class="prophet-card" style="text-align:center;">'
                sh_html += '<span class="prophet-label">SHARPE RATIO (ANNUALIZED)</span><br>'
                sh_html += '<span class="prophet-price-sm" style="color:' + sharpe_color + ';">' + f"{bt_sharpe:.2f}" + '</span><br>'
                sh_html += '<span class="prophet-data-muted" style="font-size:0.7rem;">Above 1.0 = good | Above 2.0 = excellent</span>'
                sh_html += '</div>'
            ...
            with sh_col2:
                ...
                ad_html = '<div class="prophet-card" style="text-align:center;">'
                ad_html += '<span class="prophet-label">AVG P&L PER TRADING DAY</span><br>'
                ad_html += '<span class="prophet-price-sm" style="color:' + apd_color + ';">$' + f"{avg_per_day:+,.0f}" + '</span><br>'
                ad_html += '<span class="prophet-data-muted" style="font-size:0.7rem;">' + str(r.trading_days) + ' trading days</span>'
                ad_html += '</div>'
        ...
        if r.trades:
            ...
            if len(monthly) > 1:
                mo_html = '<div class="prophet-card">'
                mo_html += '<span class="prophet-label">MONTHLY P&L BREAKDOWN</span><br><br>'
                mo_html += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:0.3rem 1rem;font-size:0.8rem;">'
                mo_html += '<span class="prophet-data" style="color:#a0a0c0;font-weight:600;">MONTH</span>'
                mo_html += '<span class="prophet-data" style="color:#a0a0c0;font-weight:600;text-align:center;">W/L</span>'
                mo_html += '<span class="prophet-data" style="color:#a0a0c0;font-weight:600;text-align:center;">WIN%</span>'
                mo_html += '<span class="prophet-data" style="color:#a0a0c0;font-weight:600;text-align:right;">NET P&L</span>'
                for mo_key in sorted(monthly.keys()):
                    ...
                    mo_html += '<span class="prophet-data-muted">' + mo_key + '</span>'
                    mo_html += '<span class="prophet-data-muted" style="text-align:center;">' + str(stats["wins"]) + 'W / ' + str(stats["losses"]) + 'L</span>'
                    mo_html += '<span class="prophet-data-muted" style="text-align:center;">' + f"{wr:.0f}" + '%</span>'
                    mo_html += '<span class="prophet-data" style="color:' + pcolor + ';text-align:right;font-size:0.8rem;">$' + f"{stats['pnl']:+,.0f}" + '</span>'
                mo_html += '</div></div>'
...
with tab_montecarlo:
    ...
    st.markdown("""
    <div class="prophet-card prophet-card-gold">
        <span class="prophet-label"><span class="icon">🎰</span>MONTE CARLO SIMULATOR</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    if "mc_results" in st.session_state:
        ...
        if mc.profit_target_probs:
            pt_html = '<div class="prophet-card">'
            pt_html += '<span class="prophet-label">PROBABILITY OF REACHING PROFIT TARGETS</span><br><br>'
            ...
            for target, prob in mc.profit_target_probs.items():
                ...
                pt_html += '<div>'
                pt_html += '<span class="prophet-data-muted" style="font-size:0.7rem;">P(+$' + f"{target:,}" + ')</span><br>'
                pt_html += '<span class="prophet-data" style="color:' + tcolor + ';font-size:1.0rem;">' + f"{prob:.1f}" + '%</span>'
                pt_html += '</div>'
            pt_html += '</div></div>'
        ...
        with mc_ps1:
            ...
            kp_html = '<div class="prophet-card" style="border:1px solid ' + COLORS["accent_gold"] + ';">'
            kp_html += '<span class="prophet-label">OPTIMAL POSITION SIZE (HALF KELLY)</span><br><br>'
            kp_html += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0.5rem;text-align:center;">'
            kp_html += '<div>'
            kp_html += '<span class="prophet-data-muted" style="font-size:0.7rem;">RISK PER TRADE</span><br>'
            kp_html += '<span class="prophet-data" style="color:' + COLORS["accent_gold"] + ';font-size:1.0rem;">$' + f"{kelly_risk_per_trade:,.0f}" + '</span>'
            kp_html += '</div>'
            kp_html += '<div>'
            kp_html += '<span class="prophet-data-muted" style="font-size:0.7rem;">ES CONTRACTS</span><br>'
            kp_html += '<span class="prophet-data" style="color:' + COLORS["accent_gold"] + ';font-size:1.0rem;">' + f"{kelly_contracts:.1f}" + '</span>'
            kp_html += '</div>'
            kp_html += '<div>'
            kp_html += '<span class="prophet-data-muted" style="font-size:0.7rem;">MES CONTRACTS</span><br>'
            kp_html += '<span class="prophet-data" style="color:' + COLORS["accent_gold"] + ';font-size:1.0rem;">' + f"{kelly_contracts * 10:.0f}" + '</span>'
            kp_html += '</div>'
            kp_html += '</div>'
            kp_html += '<br><span class="prophet-data-muted" style="font-size:0.65rem;">'
        ...
        with mc_ps2:
            dd_html = '<div class="prophet-card">'
            dd_html += '<span class="prophet-label">EXPECTED MAX DRAWDOWN</span><br><br>'
            ...
            for label, val, color in dd_pcts:
                ...
                dd_html += '<div>'
                dd_html += '<span class="prophet-data-muted" style="font-size:0.7rem;">' + label.upper() + '</span><br>'
                dd_html += '<span class="prophet-data" style="color:' + color + ';font-size:1.0rem;">$' + f"{val:,.0f}" + '</span><br>'
                dd_html += '<span class="prophet-data-muted" style="font-size:0.65rem;">' + f"{pct_of_cap:.0f}" + '% of capital</span>'
                dd_html += '</div>'
            dd_html += '</div>'
            dd_html += '<br><span class="prophet-data-muted" style="font-size:0.65rem;">'
...
with tab_tv:
    ...
    _tv_webhook_url = "http://YOUR_IP:" + str(TV_WEBHOOK_PORT) + "/webhook"
    st.markdown(
        '<div class="pc" style="display:flex;justify-content:space-between;align-items:center;padding:0.75rem 1.5rem;flex-wrap:wrap;gap:0.5rem;">'
        ...
        '</div></div>'
        '<div style="background:#020209;border:1px solid ' + COLORS["accent_cyan"] + '44;border-radius:6px;padding:0.4rem 0.8rem;">'
        '<span class="dim" style="font-size:0.6rem;">WEBHOOK URL</span><br>'
        '<span style="font-family:JetBrains Mono,monospace;font-size:0.8rem;color:' + COLORS["accent_cyan"] + ';">'
        + _tv_webhook_url + '</span>'
        '</div></div>',
        unsafe_allow_html=True,
    )
    ...
    if alerts:
        ...
        st.markdown(
            ...
            '3. Use ngrok if running locally: ngrok http ' + str(TV_WEBHOOK_PORT) + '<br>'
...
st.markdown(
    f'<div style="text-align:center; padding:1rem; border-top:1px solid #1a1a35;">'
    f'<span class="prophet-data-muted" style="font-size:0.75rem;">'
    f'SPX PROPHET · LEGENDARY EDITION · v2.0 · Built for the {POSITION_SIZE_ES}-lot warrior'
    f'</span><br>'
    f'<span class="prophet-data-muted" style="font-size:0.65rem;">'
    f'Slope {SLOPE} pts/hr · {STOP_POINTS}-pt stops · ${DAILY_LOSS_LIMIT} daily cap'
    f'</span>'
    f'</div>', unsafe_allow_html=True)
